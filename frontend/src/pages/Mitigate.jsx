  ...
  const handleGoogleDocs = async () => {
    ...
    try {
      ...
      try {
        ...
        link.click();
        URL.revokeObjectURL(url);
        toast.success("PDF report downloaded (Google Docs unavailable on this server).");
      } catch (pdfErr) {
        alert(
          "Google Docs not configured. " +
          "PDF download also failed."
        );
      }
    } finally {
                ...
                <div className="mt-6 rounded-xl border border-amber-200 bg-white p-6">
                  ...
                  <div className="mt-4 rounded-lg bg-slate-900 p-3">
                    <p className="text-xs text-slate-400 text-center">
                      Generated by Gemini 1.5 Flash • Based on actual mitigation metrics
                    </p>
                  </div>
                </div>
              )}
